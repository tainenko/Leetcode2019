...
# with indices 0 and 1 are a pair, the nodes with indices 2 and 3 are a pair, and so 
# on. 
# 
...
#  Output: "Odd" 
# 
#  Explanation: There are 3 pairs in this linked list. Let's investigate each 
# pair individually: 
# 
...
#  Output: "Tie" 
# 
#  Explanation: There are 2 pairs in this linked list. Let's investigate each 
# pair individually: 
# 
...
#  
# 
#  Related Topics Linked List 👍 21 👎 3
...
class Solution:
    def gameResult(self, head: Optional[ListNode]) -> str:
        odd, even = head.next, head
        res = 0
        while odd:
            if odd.val > even.val:
                res += 1
            elif odd.val < even.val:
                res -= 1
            if not odd.next:
                break
            odd = odd.next.next
            even = even.next.next
        return "Odd" if res > 0 else "Even" if res < 0 else "Tie"
