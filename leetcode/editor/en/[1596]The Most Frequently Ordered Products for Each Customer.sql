...
#the mouse is the most frequently ordered product for them.
...
#frequently ordered product for them.
#John (customer 5) did not order anything, so we do not include them in the 
#result table.
# 
#
# Related Topics Database 👍 223 👎 17
...
from (
    select orders.customer_id, orders.product_id, products.product_name, rank() over ( partition by customer_id order by count(1) desc) as rk
    from orders
