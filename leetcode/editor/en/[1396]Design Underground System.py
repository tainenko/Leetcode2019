...
#  
# 
#  Related Topics Hash Table String Design 👍 3354 👎 164
...
class UndergroundSystem:
    ...
    def checkOut(self, id: int, stationName: str, t: int) -> None:
        station, start = self.times[id]
        self.avg[f"{station}_{stationName}"].append(t - start)

    def getAverageTime(self, startStation: str, endStation: str) -> float:
        return sum(self.avg[f"{startStation}_{endStation}"]) / len(self.avg[f"{startStation}_{endStation}"])
