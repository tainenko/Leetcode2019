...
# numbered jumps, and the (2ⁿᵈ, 4ᵗʰ, 6ᵗʰ, ...) jumps in the series are called even-
# numbered jumps. Note that the jumps are numbered, not the indices. 
# 
...
# j such that arr[i] <= arr[j] and arr[j] is the smallest possible value. If there 
...
# j such that arr[i] >= arr[j] and arr[j] is the largest possible value. If there 
# are multiple such indices j, you can only jump to the smallest such index j. 
#  It may be the case that for some index i, there are no legal jumps. 
#  
# 
#  A starting index is good if, starting from that index, you can reach the end 
# of the array (index arr.length - 1) by jumping some number of times (possibly 0 
# or more than once). 
# 
...
# to arr[1]. arr[3] is also the largest value, but 2 is a smaller index, so we can 
# only jump to i = 2 and not i = 3
# During our 3rd jump (odd-numbered), we jump from i = 2 to i = 3 because arr[3]
#  is the smallest value in [arr[3], arr[4]] that is greater than or equal to arr[
# 2].
