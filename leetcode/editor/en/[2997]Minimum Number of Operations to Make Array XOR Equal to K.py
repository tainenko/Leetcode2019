...
#  
# 
#  Related Topics Array Bit Manipulation 👍 576 👎 55
...
class Solution:
    def minOperations(self, nums: List[int], k: int) -> int:
        num = bin(reduce(lambda x, y: x ^ y, nums)).replace("0b", "").zfill(32)
        k = bin(k).replace("0b", "").zfill(32)
        return sum(a != b for a, b in zip(num, k))
