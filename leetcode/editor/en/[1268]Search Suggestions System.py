...
#  
# 
#  Related Topics Array String Trie 👍 4214 👎 206
...
class Solution:
    def suggestedProducts(self, products: List[str], searchWord: str) -> List[List[str]]:
        trie = self.build_trie(sorted(products))
        res = []
        for c in searchWord:
            if c in trie:
                res.append(trie[c]["suggestions"][:3])
                trie = trie[c]
            else:
                trie = {}
                res.append([])
    ...
    def build_trie(self, products: List[str]) -> Dict:
        trie = dict()
        for product in products:
            ...
            for c in product:
                if c not in curr:
                    curr[c] = dict()
                curr = curr[c]
                if "suggestions" not in curr:
                    curr["suggestions"] = list()
