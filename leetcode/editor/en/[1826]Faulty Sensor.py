...
#
#  Related Topics Array Two Pointers 👍 46 👎 38
    ...
    def badSensor(self, sensor1: List[int], sensor2: List[int]) -> int:
        ...
        for i in range(len(sensor1)):
            if sensor1[i] != sensor2[i]:
                idx = i
                break
        if idx == len(sensor1) - 1:
            return -1
        if sensor1[idx + 1:] == sensor2[idx:-1] and sensor2[idx + 1:] == sensor1[idx:-1]:
            return -1
        if sensor1[idx + 1:] == sensor2[idx:-1]:
            return 2
        elif sensor2[idx + 1:] == sensor1[idx:-1]:
            return 1
        return -1
