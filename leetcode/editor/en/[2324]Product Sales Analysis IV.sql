...
#user spent the most money. In case the same user spent the most money on two or 
...
from (
    select user_id, product_id, rank() over (partition by user_id order by sum(quantity*price) desc) rk
    from sales
