...
#  int get() Provides a number that is not assigned to anyone. Returns -1 if no 
# number is available. 
...
# phoneDirectory.get();      // It can return any available phone number. Here 
...
#  
# 
#  Related Topics Array Hash Table Linked List Design Queue 👍 333 👎 452
...
class PhoneDirectory:
    ...
    def __init__(self, maxNumbers: int):
        self.arr = [True for _ in range(maxNumbers)]
        self.idx = 0

    def get(self) -> int:
        if self.idx >= len(self.arr):
            return -1
        while not self.arr[self.idx]:
            self.idx += 1
            if self.idx >= len(self.arr):
                return -1
        self.arr[self.idx] = False
        return self.idx
