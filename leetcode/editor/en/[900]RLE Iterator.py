...
#  
# 
#  Related Topics Array Design Counting Iterator 👍 616 👎 164
...
class RLEIterator:
    ...
    def __init__(self, encoding: List[int]):
        self.idx = 0
        self.curr = 0
        self.nums = [encoding[i] for i in range(1, len(encoding), 2)]
        self.lengths = [encoding[i] for i in range(0, len(encoding), 2)]
        for i in range(1, len(self.lengths)):
            self.lengths[i] += self.lengths[i - 1]

    def next(self, n: int) -> int:
        self.curr += n
        for i in range(self.idx, len(self.lengths)):
            if self.lengths[i] >= self.curr:
                self.idx = i
                break
        if self.curr > self.lengths[-1]:
            return -1
