...
# and right children, every node of this tree has two other properties, a string
# node.val containing only lowercase English letters (possibly empty) and a non-
# negative integer node.len. There are two types of nodes in this tree:
#
#
#  Leaf: These nodes have no children, node.len = 0, and node.val is some non-
# empty string.
...
#  Otherwise if node is some internal node, S[node] = concat(S[node.left], S[
# node.right]) and S[node].length = node.len.
#
#
...
# "grtaabcpoe". So S[root][5], which represents 6th character of it, is equal to "b".
#
#
#
#
...
# Explanation: In the picture below, we put an integer on internal nodes that
# represents node.len, and a string on leaf nodes that represents node.val.
# You can see that S[root] = concat(concat("abc", "efg"), concat("hij", "klm"))
# = "abcefghijklm". So S[root][2], which represents the 3rd character of it, is
# equal to "c".
#
#
#
...
#
#
#  Related Topics Tree Depth-First Search 👍 21 👎 10
