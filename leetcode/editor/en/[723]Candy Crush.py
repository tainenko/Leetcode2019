...
# has candies on top of itself, then these candies will drop until they hit a candy 
...
#  
#  
# Input: board = [[110,5,112,113,114],[210,211,5,213,214],[310,311,3,313,314],[4
# 10,411,412,5,414],[5,1,512,3,3],[610,4,1,613,614],[710,1,2,713,714],[810,1,2,1,1
...
#  
# 
#  Related Topics Array Two Pointers Matrix Simulation 👍 916 👎 456
...
class Solution:
    def candyCrush(self, board: List[List[int]]) -> List[List[int]]:
        ...
        while todo:
            ...
            for i in range(m):
                for j in range(n - 2):
                    if 0 != abs(board[i][j]) == abs(board[i][j + 1]) == abs(board[i][j + 2]):
                        board[i][j] = board[i][j + 1] = board[i][j + 2] = -abs(board[i][j])
                        todo = True
            for i in range(m - 2):
                for j in range(n):
                    if 0 != abs(board[i][j]) == abs(board[i + 1][j]) == abs(board[i + 2][j]):
