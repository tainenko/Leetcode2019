...
# 
#
# Write an SQL query to report the ID of the airport with the most traffic. The 
#airport with the most traffic is the airport that has the largest total number 
#of flights that either departed from or arrived at the airport. If there is more 
#than one airport with the most traffic, report them all. 
#
...
from
(
select airport_id, rank() over (order by sum(flights_count) desc) rk
from cte
group by airport_id
) tmp
where rk=1
