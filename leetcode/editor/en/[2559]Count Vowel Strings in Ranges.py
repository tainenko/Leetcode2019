...
# present at the indices ranging from li to ri (both inclusive) of words that start and 
# end with a vowel. 
# 
...
#  
# 
#  Related Topics Array String Prefix Sum 👍 1129 👎 69
...
class Solution:
    def vowelStrings(self, words: List[str], queries: List[List[int]]) -> List[int]:
        vowels = [0] + list(
            itertools.accumulate([1 if word[0] in 'aeiou' and word[-1] in "aeiou" else 0 for word in words]))
        return [vowels[j + 1] - vowels[i] for i, j in queries]
